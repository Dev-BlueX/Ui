...
function GuiFunc:ToggleUI()
    ...
    ImageLabel.Image = "rbxassetid://" .. GuiConfig.Image
...
SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for _, option in pairs(ScrollSelect:GetChildren()) do
        if option.Name == "Option" then
            ...
            option.Visible = query == "" or string.sub(text, 1, #query) == query
        end
    end

    -- Cập nhật lại CanvasSize khi filter
    local OffsetY = 25 -- giữ khoảng cho SearchBox
